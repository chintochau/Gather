...
class OldHomeViewController: UIViewController{
    ...
    private func createViewModels(){
        ...
        selfEvents.forEach { event in
            viewModels[0].append(EventCellViewModel(with: event))
        }
        events.forEach { event in
            viewModels[1].append(EventCellViewModel(with: event))
        }
...
// MARK: - CollectionView
extension OldHomeViewController: UINavigationControllerDelegate{
...
extension OldHomeViewController:UICollectionViewDelegate,UICollectionViewDataSource,HomeSectionHeaderReusableViewDelegate {
